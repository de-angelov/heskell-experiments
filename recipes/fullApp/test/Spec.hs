...
collectLogs :: LogCollection -> Build -> IO (LogCollection, [Log])
collectLogs collectLogs build = undefined
...
testLogCollection runner = do 
  ...
  build <- runner.prepareBuild $ makePipeline
              [ makeStep "Long step" "ubuntuu" ["echo hello", "sleep 2", "echo world"]
              , makeStep "Echo Linux" "ubuntuu" ["uname -s"]
              ]
...
testSharedWorkspace docker runner = do 
  build <- runner.prepareBuild $ makePipeline
                  ...
                  , makeStep "Read file" "ubuntu" ["car test"]
                  ]
...
checkBuild :: JobHandler.Service -> Agent.BuildNumber -> IO ()
...
testWebhookTrigger = 
  runServerAndAgent $ \handler -> do 
    ...
    checkBuild handler $ Agent.BuildNumber (round number)
