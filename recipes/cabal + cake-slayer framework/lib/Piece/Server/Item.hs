...
getItemsHandler :: ( WithDb env m , WithLog env m ) => m [WithId Item]
getItemsHandler = do 
...
createItemHandler :: (WithDb env m, WithLog env m, WithError m) => Item -> m (Id Item)
...
deleteItemHandler :: (WithDb env m, WithLog env m, WithError m) => Id Item -> m ()
